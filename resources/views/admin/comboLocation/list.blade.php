...
<!-- ===== START: SEARCH FORM ===== -->
<form id="formSearch" method="get" action="{{ route('admin.comboLocation.list') }}">
    <div class="searchBox">
            ...
            <tbody>
                ...
                if(!empty($list)&&$list->isNotEmpty()){
                    ...
                    foreach($list as $item){
                        echo view('admin.comboLocation.oneRow', ['item' => $item, 'no' => $i]);
                        ++$i;
                    }
                }else {
...
<!-- Nút thêm -->
<a href="{{ route('admin.comboLocation.view') }}" class="addItemBox">
    ...
    <script type="text/javascript">
        function deleteItem(id){
            if(confirm('{{ config("admin.alert.confirmRemove") }}')) {
                $.ajax({
                    url         : "{{ route('admin.comboLocation.delete') }}",
                    type        : "GET",
                    dataType    : "html",
                    data        : { id : id }
                }).done(function(data){
