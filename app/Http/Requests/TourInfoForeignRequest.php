...
class TourInfoForeignRequest extends FormRequest
{
    /**
                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ...
                                        ->select('seo.slug', 'tour_info_foreign.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('tour_info_foreign_id'))) $fail('Dường dẫn tĩnh đã trùng với một Chi tiết tour khác trên hệ thống!');
                        }
                    }
                }
            ],
