...
class AirDepartureRequest extends FormRequest
{
    /**
                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ->join('air_departure', 'air_departure.seo_id', '=', 'seo.id')
                                        ->select('seo.slug', 'air_departure.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('air_departure_id'))) $fail('Dường dẫn tĩnh đã trùng với một Điểm khởi hành bay khác trên hệ thống!');
                        }
                    }
                }
            ],
