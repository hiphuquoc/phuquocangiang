...
class ComboLocationRequest extends FormRequest
{
    /**
                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ...
                                        ->select('seo.slug', 'combo_location.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('combo_location_id'))) $fail('Dường dẫn tĩnh đã trùng với một Điểm đến Combo khác trên hệ thống!');
                        }
                    }
                }
            ],
