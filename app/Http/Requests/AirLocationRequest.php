...
class AirLocationRequest extends FormRequest
{
    /**
                ...
                function($attribute, $value, $fail){
                    ...
                    if(!empty($slug)){
                        $dataCheck  = DB::table('seo')
                                        ->join('air_location', 'air_location.seo_id', '=', 'seo.id')
                                        ->select('seo.slug', 'air_location.id')
                                        ->where('slug', $slug)
                                        ->first();
                        if(!empty($dataCheck)){
                            if(empty(request('air_location_id'))) $fail('Dường dẫn tĩnh đã trùng với một Điểm đến tàu khác trên hệ thống!');
                        }
                    }
                }
            ],
