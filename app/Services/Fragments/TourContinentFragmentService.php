...
class TourContinentFragmentService implements PageFragmentProvider
{
    use BuildsFragmentUrl;

    public const PAGE_TYPE       = 'tour-continent';
    ...
    public function loadBySeoId(int $seoId): ?TourContinent
    {
        return TourContinent::query()
            ->where('seo_id', $seoId)
            ->with([
                'seo',
                'tourCountries.tours.infoTourForeign' => fn ($q) => $q->where('status_show', 1),
     ...
     */
    public function schemaOfferPrices(TourContinent $item, string $locale): array
    {
        $arrayPrice = [];
    ...
    public function toursForList(TourContinent $item): Collection
    {
        $data = new Collection();
        foreach ($item->tourCountries as $tourCountry) {
            foreach ($tourCountry->tours as $tour) {
    ...
    public function airsForList(TourContinent $item): Collection
    {
        $data = new Collection();
        $i    = 0;
    ...
    public function servicesForList(TourContinent $item): Collection
    {
        $data = new Collection();
        $i    = 0;
    ...
    {
        if (!$item instanceof TourContinent || !in_array($section, self::SECTIONS, true)) {
            return null;
        }

        return match ($section) {
            self::SECTION_TOURS => view('main.tourContinent.fragments.tours', [
                'item' => $item,
